...
async function createTrelloCard({
  ...
  if (!trelloKey || !trelloToken || !trelloListId) {
    throw new Error(
      "Faltan variables de Trello en Vercel: TRELLO_KEY, TRELLO_TOKEN o TRELLO_LIST_ID."
    );
  }
...
`.trim();
  ...
  if (!response.ok) {
    console.error("Error Trello:", data);

    throw new Error(
      data?.message ||
        data?.error ||
        "Trello rechazó la creación de la tarjeta."
    );
  }
  ...
  try {
    ...
    if (!mpResponse.ok) {
      ...
      return NextResponse.json(
        {
          error:
            mpData?.message || "Mercado Pago rechazó la creación del pago.",
        },
        { status: 500 }
      );
    }
    ...
    try {
      ...
      console.error("Error creando tarjeta en Trello:", trelloError);

      return NextResponse.json(
        {
          error:
            "El pedido se guardó en Supabase, pero Trello falló: " +
